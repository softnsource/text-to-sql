...
#         columns, rows = await self._humanize_and_filter_columns_dynamic(columns, rows, question)

#         # Existence intent → just confirm yes/no with brief description, no table dump
#         if response_intent == "existence":
#             summary = await self._humanize_existence(question, rows, columns, total)
...
#         if total <= 3:
#             summary = await self._humanize_rows(question, rows, columns)
#             return FormattedResponse(
#                 mode="conversational",
#                 text_summary=summary,
#                 table_data=rows,
...
#         if total <= 20:
#             summary = await self._generate_mixed_summary(question, rows, columns, stats, total)
#             return FormattedResponse(
#                 mode="mixed",
#                 text_summary=summary,
#                 table_data=rows,
